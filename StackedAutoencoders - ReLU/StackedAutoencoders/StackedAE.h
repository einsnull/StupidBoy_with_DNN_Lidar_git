...
typedef enum {
	SPARSE_AE,DENOISING_AE,RELU_AE
}AE_TYPE;
...
public:
	...
	void setAE_Type(AE_TYPE aeType)
	{
		this->aeType = aeType;
	};
private:
		...
		MatrixXd &data,MatrixXi &labels,double lambda)
{
	if(aeType == RELU_AE)
	{
		...
		MatrixXd delta3 = (softMaxTheta.transpose() * delta4).cwiseProduct(ReLU_Gradient(z3));
		MatrixXd delta2 = (aeTheta2.transpose() * delta3).cwiseProduct(ReLU_Gradient(z2));
		...
		double beta[],double sparsityParam[])
{
	...
	else if (aeType == RELU_AE)
	{
		//stacked denoising autoencoders
		cout << "PreTraining with  ReLU denoising autoencoder ..." << endl;
		//train the first denoising autoencoder
		ReLU_Autoencoder ae1(ndim,ae1HiddenSize);
		cout << "PreTraining ae1 ..." << endl;
		ae1.train(data,lambda[0],alpha[0],maxIter[0],miniBatchSize[0],noiseRatio[0]);
		...
		//train the second denoising autoencoder
		MatrixXd ae1Features = feedForward(aeTheta1,aeB1,data);
		ReLU_Autoencoder ae2(ae1HiddenSize,ae2HiddenSize);
		cout << "PreTraining ae2 ..." << endl;
		ae2.train(ae1Features,noiseRatio[1],alpha[1],maxIter[1],miniBatchSize[1],noiseRatio[1]);
