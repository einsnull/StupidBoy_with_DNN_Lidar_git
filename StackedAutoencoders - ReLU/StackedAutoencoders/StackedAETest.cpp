...
int main()
{
	...
	stackedAE.preTrain(trainingData,trainingLabels,lambda,alpha,miniBatchSize,
		maxIter,RELU_AE,noiseRatio,beta,sparsityParam);
	...
	char str[100];
	sprintf_s(str,"Accurancy on testset(before): %lf %%",acc1);
	...
	sprintf_s(str,"Accurancy on trainingset(before): %lf %%",acc4);
	printToFile("LogFile.txt",str);
	...
	sprintf_s(str,"Accurancy on testset(after): %lf %%",acc2);
	printToFile("LogFile.txt",str);
	MatrixXi pred3 = stackedAE.predict(trainingData);
	double acc3 = stackedAE.calcAccurancy(trainingLabels,pred3);
	cout << "Accurancy on trainingset: " << acc3 * 100 << "%" << endl;
	sprintf_s(str,"Accurancy on trainingset(after): %lf %%",acc3);
	printToFile("LogFile.txt",str);
