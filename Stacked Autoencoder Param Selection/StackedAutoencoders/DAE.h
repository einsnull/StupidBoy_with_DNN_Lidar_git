...
double binCompare(double x)
{
	return x > 0;
}

// denoising auto encoder class
class DAE : public FunctionBase
{
public:
	//weights
	MatrixXd theta1;
...
private:
	...
	void miniBatchSGD(MatrixXd &trainData,double noiseRatio,
		double alpha,int maxIter,int batchSize);
	double computeCost(MatrixXd &data,double noiseRatio,MatrixXd &theta1Grad,
...
double DAE::computeCost(
	MatrixXd &data,double noiseRatio,
...
void DAE::miniBatchSGD(
	MatrixXd &trainData,double noiseRatio,double alpha,int maxIter,int batchSize)
{
	...
	for(int i = 0; i < maxIter;i++)
	{
		...
		for(int j = 0;j < numBatches; j++)
		{
			...
			J += computeCost(miniTrainData,noiseRatio,theta1Grad,theta2Grad,
				b1Grad,b2Grad);
#ifdef _IOSTREAM_
	...
	int miniBatchSize)
{
	...
	miniBatchSGD(trainData,noiseRatio,alpha,maxIter,miniBatchSize);
}
